...
const advantages = [
  ...
  {
    name: "Fuld overblik",
    desc: "Jeg kender dit projekt fra dag ét — du behøver ikke forklare konteksten igen hver gang du kontakter os.",
    icon: (
      <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
    ...
    <section className="bg-[#111313] border-t border-[#1e2020] px-5 sm:px-10 lg:px-20 py-20">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <AnimatedInView
            as="div"
            className="bg-[#1c1e1e] border border-[#2a2d2d] rounded-2xl p-7 flex flex-col gap-7"
          >
            {advantages.map(({ name, desc, icon }) => (
              <div key={name} className="flex items-start gap-3">
                <div className="w-8 h-8 rounded-lg bg-[#1a2a30] flex items-center justify-center flex-shrink-0 mt-0.5">
                  {icon}
                </div>
                <div>
                  <p className="text-[14px] font-medium text-[#e0e0e0] mb-1">{name}</p>
          ...
          <AnimatedInView
            as="div"
            className="bg-[#1c1e1e] border border-[#2a2d2d] rounded-2xl p-7 grid grid-cols-2 gap-px"
          >
            <div className="pr-7">
              ...
              <div className="flex flex-col gap-2.5">
                {bureauRows.map((row) => (
                  <div key={row} className="flex items-center gap-2">
                    <span className="w-4 h-4 rounded-full border border-[#333] bg-[#1e1e1e] flex-shrink-0" />
            ...
            <div className="pl-7 border-l border-[#2a2d2d]">
              ...
              <p className="text-[13px] font-medium text-[#e0e0e0] mb-4">
                Sådan ser det ud her
              </p>
              <div className="flex flex-col gap-2.5">
                {direkteRows.map((row) => (
                  <div key={row} className="flex items-center gap-2">
                    <span className="w-4 h-4 rounded-full border border-[#0a5a7a] bg-[#1a2a30] flex items-center justify-center flex-shrink-0">
                      <CheckIcon />
                    </span>
